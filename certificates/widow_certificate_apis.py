...
def create_widow_certificate(
    ...
    if cert.district_id and cert.taluka_id and cert.gram_panchayat_id:
        barcode_dir = os.path.join(UPLOAD_DIR, str(cert.district_id), str(cert.taluka_id), str(cert.gram_panchayat_id), "widow_certificates", "barcodes", str(cert.id))
    else:
        barcode_dir = os.path.join(UPLOAD_DIR, "widow_certificates", str(cert.id))
...
def get_widow_certificate(id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if cert.district_id and cert.taluka_id and cert.gram_panchayat_id:
        cert_data.barcode_url = str(request.base_url)[:-1] + f"/certificates/widow_barcode/{cert.id}?district_id={cert.district_id}&taluka_id={cert.taluka_id}&gram_panchayat_id={cert.gram_panchayat_id}"
    else:
...
def get_widow_certificate_barcode(
    id: int, 
    ...
    if district_id and taluka_id and gram_panchayat_id:
        ...
        barcode_path = os.path.join(UPLOAD_DIR, str(district_id), str(taluka_id), str(gram_panchayat_id), "widow_certificates", "barcodes", str(id), "barcode.png")
    elif cert.district_id and cert.taluka_id and cert.gram_panchayat_id:
        # Use certificate's stored location
        barcode_path = os.path.join(UPLOAD_DIR, str(cert.district_id), str(cert.taluka_id), str(cert.gram_panchayat_id), "widow_certificates", "barcodes", str(id), "barcode.png")
    else:
        # Fallback to old path
        barcode_path = os.path.join(UPLOAD_DIR, "widow_certificates", str(id), "barcode.png")
    
    # Check if file exists in new location, if not try to migrate from old location
    if not os.path.exists(barcode_path):
        old_barcode_path = os.path.join(UPLOAD_DIR, "widow_certificates", str(id), "barcode.png")
...
@router.put("/widow/{id}/fix-location")
def fix_widow_certificate_location(
    id: int,
    ...
    old_barcode_path = os.path.join(UPLOAD_DIR, "widow_certificates", str(id), "barcode.png")
    new_barcode_dir = os.path.join(UPLOAD_DIR, str(district_id), str(taluka_id), str(gram_panchayat_id), "widow_certificates", "barcodes", str(id))
...
@router.get("/widow-all")
def list_all_widow_certificates(db: Session = Depends(get_db)):
    """Get all Widow certificates without filters for debugging"""
    certificates = db.query(WidowCertificate).all()
